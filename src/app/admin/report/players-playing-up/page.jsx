      ...
      <tbody>
        ...
        <tr className={'my-2'} key={index}>
          <td className={'p-2 border border-stone-300'}>
            <Link target={'_blank'} className={linkStyles.join(' ')} href={`/admin/fixture/${playingUp.fixtureId}`}>{playingUp.fixtureId}</Link>
          </td>
          <td className={'p-2 border border-stone-300'}>
            <Link target={'_blank'} className={linkStyles.join(' ')} href={`/admin/player/${playingUp.player.id}`}>{playingUp.player.name}</Link>
          </td>
          <td className={'p-2 border border-stone-300'}>
            <Link target={'_blank'} className={linkStyles.join(' ')} href={`/result/${yearName}/team/${playingUp.team.slug}`}>{playingUp.team.name}</Link>
          </td>
        </tr>
      ))}
