  ...
  return (
    <FrontLayout>
      <div className='max-w-[768px] mx-auto'>
        ...
        <p className='my-6 mb-12'>The League now offers an alternative to paying a fee for each practice session at Hyndburn Table Tennis Centre or St. Peters in Burnley. Please find out more on the <GeneralLink href='/prepaid-practice-scheme' className={linkStyles.join(' ')}>Prepaid Practice Scheme</GeneralLink> page.</p>
            ...
            {Object.entries(venueSessions).map(([venueId, sessions]) => (
              <div key={venueId} className='mb-4'>
                <h3 className='text-lg mb-4'>
                  <GeneralLink href={`/result/${currentYear.name}/venue/${sessionVenues[venueId].slug}`} className={linkStyles.join(' ')}>
                ...
                <ul className='list-disc list-inside pl-5'>
                  {sessions.map((session, index) => (
                    <li key={index}>
                      ...
                      {session.contactId && (
                        <span className='ml-2 text-sm text-gray-500 text-base'>Contact: <GeneralLink className={linkStyles.join(' ')} href={`result/${currentYear.name}/player/${sessionContacts[session.contactId].slug}`}>{sessionContacts[session.contactId].name}</GeneralLink></span>
                      )}
                    </li>
                  ))}
                </ul>
